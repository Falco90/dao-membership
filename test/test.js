...
describe("Bounty Hunter Game", function () {
  it("Should create NFTs and give NFTs and ERC20 tokens as rewards to players", async function () {
    const Game = await ethers.getContractFactory("BountyHunterDAO");
    ...
    await game
      ...
      .completeContract(nftContractAddress, 1, erc20ContractAddress, badgeContractAddress);

    let activeContracts = await game.fetchContracts();

    activeContracts = await Promise.all(activeContracts.map(async c => {
    ...
    completedContracts = await Promise.all(completedContracts.map(async c => {
      const tokenURI = await nft.tokenURI(c.tokenId);
      let contract = {
        tokenId: c.tokenId,
        reward: c.reward,
